...
impl Blockstore {
    ...
    pub fn write_vote_signature(&self, slot: Slot, signature: VoteSignatureMeta) -> Result<()> {
        self.vote_signatures_cf.put(slot, &signature)?;
        Ok(())
    }

    pub fn read_vote_signature(&self, slot: Slot) -> Result<Option<VoteSignatureMeta>> {
        self.vote_signatures_cf.get(slot)
    }

    pub fn read_all_vote_signatures_for_slot(&self, slot: Slot) -> Result<Vec<Signature>> {
